...
No missing, automatic-fallback, skip, or manually authored certificate mode exists. The GitHub deployment workflow proves workflow evidence through the built-in `GITHUB_TOKEN` and does not rerun either suite. Local mode revalidates exact HEAD, `origin/main` containment and worktree cleanliness before and after running both suites, then continues directly into the existing protected path.
