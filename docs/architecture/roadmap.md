...
- 🟡 RMVP-06B connected Confirmed Need validation implementation — implemented on bounded draft branch `codex/rmvp-06b-connected-confirmed-need-validation` from accepted baseline `2ac4775af957070225916da3912c61df30ae4395`; exact-head GitHub validation, product/architecture review, and merge remain pending. The implementation adds one command, three immutable evidence relations, the complete 19-blocker/two-warning registry, additive read-only validation UI, 65 focused pgTAP assertions, and short/full browser verification; [API](../api/rmvp-06-connected-confirmed-need-validation.md) and [implementation record](../implementation-tasks/TASK-RMVP-06B-connected-confirmed-need-validation.md). Approval, release, reopen, CMD-03, downstream domains, hosted deployment, production capability binding, and Retool remain excluded.
...
- 🟡 React connection through RMVP-06B — the existing sixth Planning Inputs tab now includes complete-batch validation status, blockers/warnings, line markers, and read-only success state on the draft branch; merge remains pending
