...
- ✅ ATLAS-MODEL-PRINCIPLE-01 — [facts explicit, state derived, supporting objects generated](../decisions/decision-atlas-model-convergence.md), with the [current authority map through Procurement](atlas-authority-map-through-procurement.md)
...
- ✅ UI-QUALITY-03C-B Ingredient/Supplier operator workflow hardening — merged through PR #196 at `3376be46c9f061a91a662873faf67024fd50b233`; preserves the existing tabs/search/catalog/detail structure, removes the hidden 60-result cap, adds exact canonical Review before authored `RMVP-01.v1` writes, restores two predefined private authoritative Ingredient catalogs and API-backed selects, corrects `Nhóm đặt hàng`/rounding wording, retains lifecycle confirmation, and adds stale/unknown/readback recovery with no Planning delta
- ✅ PLANNING-UX-01B Menu/Attendance operator correction — merged through PR #197 at `80cdc2d`; Menu-covered Attendance defaults appear automatically as unconfirmed working proposals, persisted values including zero win, Review remains required before consequential Save, and the manual default-creation action is removed
- ✅ PURCHASE-REVIEW-CONFIRM-RELEASE-01 — merged through PR #251 at `a398c22`; generated review is advisory, exact saved supplier splits are explicit decisions, preparation atomically composes existing release/Handoff/promotion/draft boundaries, and each PO release remains an independent supplier commitment
- ✅ RECIPE-EFFECTIVE-CONTRACT-01 backend contract — merged through PR #257 at `a60085163ecbfde8dc5f7c2d97a454bc57ec0f60`; canonical typed system/School reads, base-authoring context, stable effective targets, backend history and atomic two-scope Dish copy are present
- 🟡 ATLAS-MODEL-CONVERGENCE-01 — Draft PR #258 is the single integrated repository candidate through Procurement: certified A07/A12 migrations, exact SYSTEM_DISH context, per-revision issuance truth and bounded Application adoption are 56/56 locally accepted. It remains unmerged and un-deployed; Staging is not ready and Warehouse has not started.
...
→ PLANNING-CONTRACT-02A source-replacement evidence
→ PLANNING-CONTRACT-02B selective confirmation continuity
→ UI-QUALITY-02AB-UX
→ UI-QUALITY-02C-A
→ UI-QUALITY-02C-B
...
PLANNING-UX-01B implemented the Attendance operator correction. Menu School/date coverage automatically supplies default-derived working rows; persisted Attendance overlays matching pairs, including explicit zero, and only missing covered pairs use defaults. The rows remain unconfirmed until reviewed and saved, dirty local work is protected, and later School-default changes do not rewrite persisted Attendance. The former `Tạo từ sĩ số mặc định` action is no longer part of the normal UI. See the [implementation record](../implementation-tasks/TASK-PLANNING-UX-01B-menu-attendance-operator-correction.md).
...
- ✅ PLANNING-CONTRACT-02B selective Confirmed Need continuity — merged through PR #200, reconciled to the frozen security catalog through PR #201, and locally certified by PLANNING-ACCEPTANCE-01; adds private immutable carry/invalidation evidence, direct-or-carried decision authority, additive backend classifications/counts, and bounded Application presentation without public API/capability/lifecycle or D-039 semantic changes; [architecture](planning-contract-02b-selective-confirmation-continuity.md), [implementation record](../implementation-tasks/TASK-PLANNING-CONTRACT-02B-selective-confirmation-continuity.md), and [acceptance record](../implementation-tasks/TASK-PLANNING-ACCEPTANCE-01-local-integrated-acceptance.md)
...
- 🟡 PA-06E Confirmed Need review, adjustment, revision, and source-correction documentation — [contract](pa-06e-confirmed-need-review-adjustment-revision-contract.md) and [decision](../decisions/decision-pa-06e-confirmed-need-source-correction.md); D-040 is the accepted selective-continuity amendment, while all other pending product decisions remain unapproved
...
- ✅ Hosted Supabase alignment for the merged Planning path — Atlas Staging is aligned through `ATLAS-STAGING-ALIGN-01` at exact certified `main` `6d2a2651f7aebf8e9cdba6a17432a35f9dee31a0`; hosted migration history is 44/44 and the protected platform verifier passed. Hosted operator/security rehearsal remains separate.
...
- ✅ Connected school-catering Procurement backend and UI — Handoff/allocation and PO draft/release changes merged through PRs #238, #240, #241 and #242; PURCHASE-REVIEW-CONFIRM-RELEASE-01 then aligned generated review, confirmed-source allocation and atomic preparation through PR #251 while preserving independent PO release
...
PA-01 through PA-03 are approved documentation and architecture baselines. PA-04 is the merged first executable database foundation. PA-05A, PA-05B, PA-05C, PA-05B-H1, PA-05D, PA-05E, PA-05B-H2, PA-05F, PA-05B-H3, and PA-05C-H2 are completed on `main`; PA-05G is implemented by the bounded acceptance change with 82 focused assertions. The reviewed wholesale API proves one command-authored source-to-successful-trip-closure path plus trace, readiness, blockers, and current aggregate-compatible audit reads. School-catering Procurement is a separate NEED_GENERATION path: PR A owns Handoff and Allocation Families, while PR B reuses the shared PO aggregate for reviewed supplier/date draft and release behavior. Controlled production seed/reference data, legacy rehearsal, deployment, Warehouse, and broader write-command coverage remain separately approved work.
