...
- 🟡 RMVP-04 connected Need Generation — one exact-period Planning workbench, create/validate/release/invalidate commands, existing CMD-15 connection, and a fifth Vietnamese Planning Inputs tab are implemented on a bounded draft branch; [API contract](../api/rmvp-04-connected-need-generation.md) and [implementation record](../implementation-tasks/TASK-RMVP-04-connected-need-generation.md). GitHub exact-head validation and merge remain pending; Procurement, Warehouse, Dispatch, hosted deployment, and production rollout remain outside scope.
