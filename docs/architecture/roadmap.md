...
- 🟡 RMVP-03B Planning Input Readiness — the exact-period root, immutable evaluation/issues, closed lifecycle, and three-source Pantry binding are implemented through PANTRY-RDY-02 on the bounded task branch; commands/UI remain not started
...
- 🟡 PANTRY-RDY-02 readiness persistence amendment — implemented and locally validated on `backend/pantry-rdy-02-readiness-persistence`, pending draft-PR review and merge; one migration, zero new relations/APIs/functions/triggers/capabilities/roles/scope kinds/policies/grants/source triggers, and the same three readiness suites at exact plans `36/59/57`; [decision](../decisions/decision-pantry-rdy-02-readiness-persistence.md) and [task](../implementation-tasks/TASK-PANTRY-RDY-02-readiness-persistence.md)
