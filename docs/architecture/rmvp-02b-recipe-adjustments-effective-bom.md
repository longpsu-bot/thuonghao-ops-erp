...
The Dish must be active and require Need Generation. The selected Recipe is active and its version is `RELEASED_FOR_PLANNING`. Multiple candidates in the selected tier block. A caller cannot choose an arbitrary version for a current result.
