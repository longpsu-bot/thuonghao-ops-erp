...
These are existing identities at the evidence baseline, not requests to introduce replacement APIs. Verify signatures at execution time. Catalog existence does not prove current human-role authorization.
...
| Create Dish                                      | `create_dish` — RMVP-02A.v1                                                                                                                                                 | ACTIVE Dish + two roots; no RecipeVersion.                                                                           |
...
| Preview / create / correct / cancel Change Order | Existing `preview_recipe_composition_adjustment`, `create_recipe_composition_adjustment`, `supersede_recipe_composition_adjustment`, `cancel_recipe_composition_adjustment` | Preserve existing envelopes and scope semantics; verify system preview support rather than inventing a proxy School. |
| Read Change Order ledger                         | `get_recipe_adjustment_operator_workbench` — RMVP-02B.v2                                                                                                                    | Server-shaped temporal state/history; do not replay raw revisions.                                                   |
