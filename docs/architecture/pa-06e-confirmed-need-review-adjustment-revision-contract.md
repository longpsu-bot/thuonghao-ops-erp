...
**Connected H1 implementation status:** RMVP-05 read/preview/quantity confirmation is implemented on a bounded draft branch; [exact API registry](../api/rmvp-05-connected-confirmed-need-review.md) and [implementation record](../implementation-tasks/TASK-RMVP-05-connected-confirmed-need-review.md). Complete-batch validation, approval, release, CMD-03, and downstream correction remain unimplemented.
...
**Operational proposal decision:** [D-046 - Policy-derived Planning Operational Proposal](../decisions/decision-planning-operational-proposal.md)
...
→ Draft Confirmed Need line revision
   ├─ theoretical_quantity = exact grouped raw requirement
   └─ confirmed_quantity = policy-derived operational proposal
...
→ sets exact grouped theoretical_quantity
→ resolves the exact effective Unit/service-date Planning policy
→ writes confirmed_quantity as ceil(theoretical_quantity / planning_step)
  * planning_step, a clearly non-authoritative proposal
...
materializer aggregates the complete operational group before one exact upward
quantization. It never quantizes individual contributions. Zero, ineffective,
or multiple eligible policy revisions for any grouped exact Unit and service
date reject the whole command before a Confirmed Need write. System derivation
creates no H1B1 decision; later human input remains subject to exact-tick
rejection without a replacement value.
...
| PA-06E-H1 | One-line authorized read, backend preview, and explicit confirmation against the H0-generalized schema                                                     | `get_confirmed_need_review`, `preview_confirmed_need_confirmation`, `confirm_need_quantities`                                                                            | Implemented by bounded RMVP-05 draft; exact registry linked above                   |
