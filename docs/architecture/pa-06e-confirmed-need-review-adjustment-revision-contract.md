...
H0 must design, migrate, secure, and test a governed physical representation of the existing logical source and decision-evidence children. Documentation-level source-reference alternatives are:

| Alternative                                            | Direction                                                                                                                                                                                            | Assessment                                                                                                                 |
| ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| A — one dedicated `ConfirmedNeedSourceReference` child | Link a stable line/revision to an exact Need Generation line/run and its governed source revision set.                                                                                               | Small and close to the business contract, but still needs typed relational detail for heterogeneous source families.       |
| B — governed typed source-reference relations          | Use declared Recipe/Menu/Attendance/Need Generation foreign keys or typed child bridges with exactly-one-source constraints.                                                                         | Most consistent with PA-02 and strongest relational validation, but can create more bounded tables.                        |
| C — bounded typed child family                         | Implement the logical `ConfirmedNeedSourceReference` as a dedicated child/evidence family whose required Need Generation identity is explicit and whose source details use declared typed relations. | Recommended direction for H0, subject to schema review; preserves one business child while retaining relational integrity. |

H0 must not use polymorphic free-text IDs, caller-authored table names, generic unvalidated JSON lineage, fake wholesale records, or a second Confirmed Need aggregate. Exact tables, constraints, backfill behavior, compatibility views, grants, RLS, and command ownership remain pending and require a separate approved migration task.
...
| Technology          | H0 must first generalize the wholesale-only PostgreSQL source lineage and decision evidence; future PostgreSQL commands own arithmetic, validation, versions, revisions, events, and audit. |
...
This read cannot be implemented for a genuine school-catering line against the current wholesale-only PA-04 schema. H0 must first supply governed source references and line decision evidence.
...
H0 is the prerequisite architecture/security/migration contract for:

- removing the school-catering dependency on required wholesale-only source FKs without breaking `PA-05D.v1`;
- physically representing governed `ConfirmedNeedSourceReference` lineage using typed relations;
- physically representing line-level confirmation/adjustment evidence, including unchanged-proposal acceptance;
...
PA-06E does not authorize H0 DDL, commands, or migration. H0 must explicitly prove that direct wholesale remains exact and that no fake wholesale record can satisfy a school-catering path.
...
- governed source-reference rows bind one exact generation result and typed Recipe/Menu/Attendance source revisions;
- no caller-authored table name, polymorphic free-text ID, or generic unvalidated JSON lineage is accepted;
- `create_confirmed_needs_from_generation` materializes exact theoretical and proposed values but creates no Planning-confirmed authority;
...
- line decision evidence records actor/time, reason, before/after, rule/source set, revision, batch version, command, and unchanged acceptance; and
...
9. **First backend boundary:** H0 must first generalize source lineage/decision evidence and contract materialization; H1 then proves one synthetic line, policy, preview, confirmation, result, readback, line evidence, event/audit/receipt, with no validation, approval, release, handoff, Procurement, or production queue.
...
12. **Remaining product/architecture decisions:** exact typed source-reference schema, line decision-evidence physical shape, unchanged-acceptance reason rules, H0 migration/compatibility/security, exact Planning policy/steps and owner, zero-line release behavior, prior-confirmed carry proposal, separation of duties, service-date authorization, purchase-advisory blocking, API names/capabilities, downstream correction variants, and final Vietnamese term.
