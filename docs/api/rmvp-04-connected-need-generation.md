...
`period_start` and `period_end` are not accepted in a v3 payload. The backend adapts the intent to the approved atomic implementation with `period_start = period_end = service_date`; therefore one successful command creates or advances only that date's Planning Input evaluation, Need run/release, and Confirmed Need batch. Public v2 remains callable only for exact `D..D` compatibility. A valid public v2 multi-day intent returns `MULTI_DAY_NEED_GENERATION_RETIRED`; the former arbitrary-period implementation remains private to the Need runtime for controlled historical/internal compatibility.
...
Confirmed Need navigation uses the same seven date-scoped preflight reads after reload or a new session. React may present those backend-owned daily states as a weekly date selector and pass the selected exact `confirmed_need_batch_id` to the existing review read; it does not select one week-level current Need, merge batches, or derive currentness.
...
at least one Menu snapshot line for D or one positive Pantry snapshot line for
D. Attendance alone and an approved zero-line Pantry snapshot do not create
demand. When neither demand source exists and there is no current exact-day
Need, preflight returns `BLOCKED` with
