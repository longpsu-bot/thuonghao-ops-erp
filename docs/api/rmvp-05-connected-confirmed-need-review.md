...
`scripts/verify-local-rmvp05-confirmed-need-review.mjs` is GitHub-only. With a disposable local identity and browser key, it continues the RMVP-04/CMD-15 journey through review, mixed exact preview, confirmation, exact replay, correction-note enforcement, replacement confirmation, and authoritative decision readback.
