...
## 8. PLANNING-CONTRACT-02B additive read and decision semantics

The `RMVP-05.v1` public surface and request envelopes remain unchanged. Current line readback adds backend-owned `confirmation_state`: `CARRIED_FORWARD`, `CHANGED`, `NEW`, `UNREVIEWED`, or `CONFIRMED_CURRENT`. Counts preserve `total`, `unreviewed`, `confirmed`, and `adjusted` and add `carried_forward`, `needs_review`, `changed`, `new`, and `removed`; removed facts are not current lines.
