...
## 4K. Closed-loop Planning source correction

D-042 adds exactly two public functions under `PLANNING-CORRECTION.v1`, taking the whole-platform registry from 90 to 92 functions. It adds no role, capability, lifecycle status, or business relation.

| Function                                                 | Kind                  | Required capability                                                                                                           | Payload and consumed versions                                                                                                                                                                                                                                                | Authoritative aggregate                                                                 | Bounded response                                                                                                                                                                                                                       |
| -------------------------------------------------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `atlas_api.get_planning_source_correction_impact(jsonb)` | Shaped read           | `planning.inputs.read`                                                                                                        | `{ source_kind: WEEKLY_MENU \| ATTENDANCE \| PANTRY, source_payload: <the exact proposed consequential-Save payload> }`; consumes current approved canonical source facts and every active Need/Confirmed Need/Handoff/later lineage containing each materially changed date | Existing Weekly Menu, Attendance, or Pantry source plus date-containing Planning chains | `impact` with `material_change`, exact `affected_service_dates`, `save_allowed`, blocker code, and per-date Need/CN state, release/Handoff/later/legacy flags, policy, next action, operator message, and all matching chains/versions |
| `atlas_api.prepare_planning_source_correction(jsonb)`    | Transactional command | `planning.need_generation.write`; additionally `confirmed_need_release.release` only when reopening a released Confirmed Need | Normal Atlas command envelope with expected Need-run version and reason `PLANNING_SOURCE_CORRECTION_PREPARED`; payload `{ need_generation_run_id, confirmed_need_batch_id, expected_confirmed_need_batch_version }`                                                          | One exact existing Need/Confirmed Need chain                                            | Either `CONFIRMED_NEED_REOPENED` or `LEGACY_RANGE_INVALIDATED`, affected IDs, new versions, event/audit IDs and safe operator message; stale, capability, Handoff, and later-commitment failures are no-write responses                |
...
## 4L. Implemented connected school-catering Procurement backend
