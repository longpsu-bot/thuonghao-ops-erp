...
One transactional backend command coordinates the existing authorized Planning release, real school-catering Handoff, allocation promotion and PO draft commands. Every child retains its own authorization and receipt checks. It can continue from already completed release/Handoff stages. Successful preparation may terminate at either exact current Draft/released PO coverage or a D-044 replacement frontier: every otherwise-uncovered current positive supplier/date requirement has an active stale released PO whose derived commitment state is `REPLACEMENT_REQUIRED`. At that frontier the corrected Planning release, Handoff successor and Handoff-source allocation promotion commit, while the old PO remains released and unchanged; creating the complete replacement Draft remains the existing separate explicit operator command. An active ordinary or stale PO Draft outside exact coverage blocks preparation. The only Draft that may coexist at the replacement frontier is a current complete replacement directly linked to the active released predecessor; rerunning preparation in that two-root state leaves both documents, their revisions, quantities and replacement lineage unchanged. `CANCELLATION_REQUIRED`, missing supplier/date coverage, a failed child, skipped date, other invalid PO state or stale allocation aborts all newly performed child work. Retryable failures also roll back the outer receipt; unknown transport outcomes require authoritative refresh, not automatic replay. Explicit retry retains the complete original request and is discarded when date/stage intent changes.
