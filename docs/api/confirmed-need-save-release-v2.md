...
An RMVP-05.v2 backend error carries the explicit reviewed
`write_certainty = NO_COMMITTED_CHANGE` contract. This includes an unexpected
exception handled after PostgreSQL has rolled back the command subtransaction;
the safe message must therefore describe a failed Save with no committed change,
...
automatically. Only a genuine `transport_error` is currently an unknown Save
outcome under a valid response. A missing or unreviewed backend certainty also
fails closed and requires authoritative refresh rather than inferring safety from
an error code, status, or retry hint. No separate uncertain backend
write-certainty value is defined by this contract.
