...
## PR #251 promoted-allocation verifier correction — 2026-09-04

Starting head: `9690d749fe1762b512c224cc09381bf64f05c153`. The canonical checkout was clean at the requested branch/head and PR #251 remained Draft. Linux [Full Integration run 33777483023](https://github.com/longpsu-bot/thuonghao-ops-erp/actions/runs/33777483023) successfully installed the canonical Procurement fixture, released Handoff and read its public workbench, then failed the legacy requirement for an `UNALLOCATED` row with a 100% recommendation. The approved flow instead promotes already-saved supplier decisions to a balanced `PURCHASE_HANDOFF` successor.

Diagnostic RED used a fresh disposable local stack and the existing `pnpm local:school-catering-procurement:verify` command. A temporary public-read diagnostic captured two post-Handoff rows: both `BALANCED`, `PURCHASE_HANDOFF`, family version **2**, and `recommendation: null`. Their sole persisted split was Supplier A (`c7100000-0000-4000-8000-000000000001`), with exact quantities **3.000000** and **2.000000**, respectively, and ratio **1.000000000000**. Handoff's shaped response returned the matching promotion identities, versions and fingerprints. The old assertion then failed with the same message as Linux. Evidence: `%TEMP%/atlas-pr251-promoted-verifier/procurement-red.log`.

The verifier now snapshots the relevant saved allocation families through `get_confirmed_supplier_allocation_workbench` before Handoff, then compares the promoted public readback and Handoff response. It asserts balanced persisted splits, exact BigInt quantity totals, preserved supplier assignments/quantities/ratios, `PURCHASE_HANDOFF` source, the same family identity with one version advance, matching promotion fingerprint/revision evidence, and absence of an authoritative recommendation. The public responses do not expose predecessor revision IDs, so no private-table query was added to manufacture that evidence.

The obsolete recommendation-confirmation command and its version-1/version-2 assumptions are removed. The retained manual 60/40 test uses the current family version and fingerprint, the existing BigInt split calculation, an exact one-version advance, and exact supplier quantities/ratios on readback. Both supplier PO drafts must reference the current family revision and persisted split IDs with their exact quantities. Existing draft eligibility, backend document-number release and released readback assertions remain intact; the 67-assertion PO suite retains immutable released-document coverage.

This is a verifier/documentation change only. No production migration, API, allocation persistence, Handoff promotion, predecessor history, eligibility, PO behavior, release guard, RLS or capability changed. There is no new migration or production rollback effect. Staging, live OPS and Retool received zero writes. No browser QA or Qodana refactor is required.

Connected GREEN on a fresh local reset completed the entire `pnpm local:school-catering-procurement:verify` journey: **2 preserved/promoted allocations**, exact manual 60/40 editing at current family version **2 → 3**, current-split supplier PO drafts, backend-number release and authoritative readback. The embedded planning-correction suite passed **18/18** and the PO suite passed **67/67**. Evidence: `%TEMP%/atlas-pr251-promoted-verifier/procurement-green.log`. No suitable existing standalone verifier test file exists; the connected RED/GREEN journey supplies the requested regression coverage without a new mock suite.

After another clean reset, `node scripts/test-local-purchase-review.mjs purchase_review_confirm_release.sql` passed **76/76**. The single required `pnpm certify:supabase:full-integration` attempt passed its first **79/80 commands**, including full-upstream RMVP-06/07, then encountered the known Windows multiline `ready_count` transport issue in command 80 before the updated promotion assertions. Its final verifier still passed planning correction **18/18** and purchase orders **67/67**, then incorrectly attempted to reinstall `rmvp_05_browser_fixture.sql`. This is a failed local certification, distinct from the successful targeted Procurement journey; no transport workaround or production change was added. Log: `%TEMP%/atlas-pr251-promoted-verifier/full-integration.log`, SHA-256 `CD76A3D0245D195361E3952514B52D22E3253D3321735C7B8B5BDCC6E58709BB`.

`pnpm typecheck`, Prettier on both touched files, and `git diff --check` passed. The local certification cleaned up its disposable stack. PR #251 remains Draft until the decisive exact-head Linux Full Integration passes; its run ID, final result and delivered head are recorded in the PR body without a self-referential source commit hash here.
