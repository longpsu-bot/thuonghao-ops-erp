...
Follow-up review at published head `4ead25e2a0066458562c02ab9052aa33d4508365` found that the deferred H0B1b evidence guard still reread the School's mutable current default for an already materialized current Recipe revision. The corrected guard now uses the theoretical line destination for Pantry, the predecessor immutable contribution destination for a Recipe successor, and the revision's captured destination for a Recipe contribution without a predecessor. CMD-15 alone continues to use the School's current default when it initially creates a genuinely new Recipe operational identity.
...
The replaced H0B1b membership guard derives the per-revision effective destination by source family: Pantry uses the theoretical line's exact destination; a Recipe successor uses its predecessor's immutable contribution destination; and a Recipe contribution without a predecessor uses the revision's captured destination. It never rereads the School's mutable current default. The guard retains nonempty every-and-only membership, exact totals, immutable membership, active-release partition completeness, and exactly-once current membership while rejecting cross-destination assignment, omission, and duplication.
...
The new focused suite is `supabase/tests/pantry_ng_02_direct_ingredient_persistence_materialization.sql` at exact `plan(144)`. Its arithmetic is `58 + 10 + 9 + 11 + 12 + 23 + 21 = 144`. The final 21 assertions now execute real CMD-15 transactional coverage for one Recipe and one Pantry contribution with the same date, School, Customer, Ingredient, and Unit but different destinations. They prove two separate stable lines and revisions, family-exact memberships, separate quantities, exactly-once partitioning, and response-count agreement. The H0B1b contribution suite remains exact `plan(80)`, exercises both rejected cross-family destination assignments, and proves that a later School-default change cannot move or invalidate an already captured Recipe stable line, revision, or contribution when a non-identity revision fact changes. Workflow registration produces the authoritative target of 36 pgTAP files and 2,178 assertions.
