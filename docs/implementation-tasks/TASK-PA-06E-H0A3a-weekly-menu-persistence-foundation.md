...
- require every root to enter as `DRAFT`, advance reopen to the next working version, and preserve established approval evidence across later transitions;
...
- accept lowercase nonblank menu-slot evidence without seeding or hard-coding a catalogue;
...
The focused test contains 74 assertions. It proves the exact four-relation schema, database-generated identities, non-Monday seven-day service periods, source and version constraints, typed assignments, stable ownership, allowed lifecycle, working-line mutability, exact active-line approval completeness, snapshot immutability, historical snapshot retention, restrictive indexed foreign keys, forced RLS, fail-closed privileges, unchanged API registry, and zero role or capability seeds.
...
- focused `supabase test db supabase/tests/pa_06e_h0a3a_weekly_menu_persistence_foundation.sql --local`: pass, 74/74 assertions;
