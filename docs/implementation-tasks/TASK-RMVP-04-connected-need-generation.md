...
Root review adds the forward migration `20260823055452_issue_223_legacy_overlap_guard.sql` and one partial active-multi-day period index matching its lookup. For D, any non-`INVALIDATED` multi-day Need run whose inclusive period contains D produces `LEGACY_OVERLAP` / `ACTIVE_LEGACY_NEED_RANGE_OVERLAP` readback and blocks all new `D..D` Planning Input, run, release, and Confirmed Need writes. Exact daily current/replay/successor behavior is unchanged; invalidated or non-containing ranges do not block. Public `RMVP-04.v2` now accepts only exact `D..D` execution and returns `MULTI_DAY_NEED_GENERATION_RETIRED` for a valid arbitrary multi-day request, while the old implementation remains private for controlled compatibility. The weekly UI displays the legacy commitment blocker on the affected row. No historical row is mutated and no Issue #222 correction path is introduced.
