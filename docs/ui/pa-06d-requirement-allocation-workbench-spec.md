...
| Nhu cầu vận hành         | Planning Operational Quantity quantized upward from grouped raw need by snapshotted `Ingredient.order_step` |
...
Normal rows keep the proposal-rounding step separate from the human
confirmation quantum. For example, raw `0,0255 kg` may yield proposal `0,1 kg`
under Ingredient step `0,1 kg`, while H1A `0,01 kg` still permits a human
confirmation such as `0,04 kg`.
...
| Lượng có ý nghĩa cho Lập nhu cầu     | Planning Operational Quantity   | Nhu cầu vận hành                            | Nhu cầu vận hành        | Lượng hệ thống đề xuất sau khi làm tròn lên một lần theo `Ingredient.order_step`, chưa phải xác nhận của con người | Số lượng vận hành chung chung   | Master `SL`/orderable                   | Phân biệt đề xuất hệ thống với bước xác nhận H1A                       | `Nhu cầu vận hành là 0,1 kg từ nhu cầu tính 0,0255 kg.` | Proposed pending product-owner and operations-language review |
