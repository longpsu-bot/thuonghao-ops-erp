...
Soften boundaries: subtle workbench edge, 6px radius, one master/detail separator, standard soft input/secondary-button borders. Use tone, spacing and typography before outlined grouping. Attached detail uses `bg.subtle`, without shadow. All routine refresh uses `AtlasRefreshButton`; all primary actions use `businessPrimary`; controls remain 40px high with an 8px radius. Workbenches must not invent alternate treatments where existing tokens/recipes describe the meaning.

Inter / Segoe UI / Arial; title 24px/650, section 17px/600, body 14px, table 13px, label 13px/600, helper 12px, button 14px/600. Spacing: 6/10/16/24/32px. Controls 40px; compact actions and refresh 36px. Normal control radius 8px; workbench radius 6px. The circular routine refresh is the intentional geometry exception.
...
**Geometry follows the operator job, not a mechanical ratio.** The 62 / 38 target is the default for a true decision master / contextual detail relationship, where the left-side master remains part of the active decision. When the left surface is only navigation and the right surface is the primary editor, use a compact fixed navigator instead (roughly 240–300px at desktop) and allocate the remaining width to the editor. Recipe authoring is the reference navigator/editor case. Homogeneity means common interaction grammar, not identical geometry.
...
**Frozen Review protects exact command intent.** While a Review/Preview represents an exact command payload against a specific authoritative snapshot, routine `AtlasRefreshButton` is unavailable. The operator closes or backs out of the frozen Review before routine Refresh. This does not forbid certified draft-preserving Refresh behavior outside Review. Recovery from read failure, stale/currentness or unknown/readback uncertainty remains explicit Vietnamese text (`Thử tải lại dữ liệu`, `Tải lại dữ liệu hiện tại`, `Tải lại để xác nhận`) and is never collapsed into the routine Refresh icon.

**Primary module motion is frozen.** Only the central workbench moves; sidebar, header, date and session context remain stationary. After the active domain authorizes exit, the outgoing workbench uses a 60ms opacity transition from `1` to `0.15`; the module is replaced; the incoming workbench uses 180ms from opacity `0.15` to `1` with `translateY(6px)` to `0`, ease-out. There is no horizontal directional slide, row/card stagger, scale, blur or two-workbench crossfade. `prefers-reduced-motion: reduce` removes the page motion and delay entirely. Dirty/UNKNOWN/stale/busy guards run before motion, so blocked navigation produces no transition.

Attached detail may use the local `detailEnter` animation: 160ms ease-out, opacity 0 to 1 and translateY(6px) to 0. Reduced motion removes this animation. This is separate from the frozen primary module transition.

## Isolation, typing and evidence

The provider owns `.atlas-vnext`; variables use `atlas` prefix, scoped preflight and scoped global styles. No production entrypoint changes. The provider supplies one reusable portal container through `useAtlasPortalContainer`; popup calendars mount under its `[data-atlas-portal-root]` inside the scoped root. Storybook retains its legacy global configuration; each new story explicitly provides the VNext provider.
...
## Procurement implementation — ATLAS-UI-VNEXT-02

The isolated Procurement workbench now applies this language to the existing confirmed supplier-allocation and purchase-commitment APIs. A reviewed five-module business bridge permits request builders, models and safe copy; legacy presentation remains forbidden. Two operator-job tabs, exact allocation detail, a scoped School picker/dirty Dialog, authoritative readback and supplier-oriented PO detail are available through vNext review stories only. Export callbacks are injected at the integration boundary. This adds no palette, persisted lifecycle, backend authority or production route. See the [implementation and verification record](../implementation-tasks/TASK-ATLAS-UI-VNEXT-02-CHAKRA-PROCUREMENT.md).
