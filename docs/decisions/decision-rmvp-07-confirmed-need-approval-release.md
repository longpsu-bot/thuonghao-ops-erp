...
| R7-P06 | Approval precondition  | Approval accepts only the exact current successful RMVP-06 validation attempt and batch version, with zero blockers, complete evidence and an equal lifecycle-neutral `RMVP-07-VALIDATED-FACTS.v1` projection.                                                                              | Approval must accept the evaluated fact set, not a mutable latest batch; the RMVP-06 attempt fingerprint includes prior lifecycle/version and cannot be recomputed after advancement.                                             |
...
| R7-P10 | Approval persistence   | Reuse existing approval snapshot and snapshot-line relations; add source kind, the exact successful validation-attempt binding, the approval-accepted `validated_fact_fingerprint`, and a source-qualified current approval pointer only.                                                   | Existing structures already model immutable approved quantities. A second approval aggregate would duplicate authority, while the lifecycle-neutral fingerprint freezes the exact fact projection accepted by approval.           |
| R7-P11 | Release persistence    | Add exactly one append-only private `NEED_GENERATION` relation `confirmed_need_releases`, constrained by typed composite ownership and bound to the exact approval snapshot.                                                                                                                | Batch fields or audit events alone cannot preserve multiple historical release cycles and provide a stable CMD-03 source; WHOLESALE must not require this relation.                                                               |
...
| R7-P13 | Source drift           | Preserve the lifecycle-bound RMVP-06 attempt fingerprint as immutable history. Approval compares current facts with the lifecycle-neutral projection reconstructed from validation evidence; release compares current facts with the exact `validated_fact_fingerprint` stored by approval. | Lifecycle transitions change batch status/version even when business facts do not. A separate projection detects real source, policy, decision, Unit, quantity, membership, blocker or warning drift without impossible equality. |
...
| R7-P15 | Read model             | Extend the existing `get_confirmed_need_review` response additively with exact typed approval/release objects, action booleans, machine-readable disabled codes, safe messages, fact-drift fields and lifecycle history; add no second read API.                                            | One workbench should show review, validation, approval and release without duplicate reads, screens or client-invented eligibility.                                                                                               |
...
| R7-P19 | Direct wholesale       | Preserve direct-wholesale `PA-05D.v1` through an explicit alternative-family rule: WHOLESALE snapshots keep null validation binding/fingerprint, require no RMVP-07 pointers or release row, and retain the atomic version-1 release; RMVP-07 commands remain `NEED_GENERATION` only.       | Prevents the school-catering lifecycle and new integrity guards from invalidating the existing wholesale shortcut.                                                                                                                |
| R7-P20 | Implementation ceiling | Future RMVP-07B: two APIs, two capabilities, zero roles/states/views/scope kinds, one new private relation, source-qualified additive columns/read/UI only.                                                                                                                                 | Establishes an exact bounded implementation and prevents workflow/platform expansion.                                                                                                                                             |
| R7-P21 | Command/read closure   | Freeze separate request reasons, note policy, exact approval/release success and failure fields, byte-for-byte receipt replay, exact additive read field names/types, closed approval/release disabled-code registries and first-match precedence.                                          | RMVP-07B must implement a reviewed surface rather than invent request, result or eligibility contracts.                                                                                                                           |
...
- source-qualified snapshot ownership;
- one exact successful-validation-attempt FK and one `validated_fact_fingerprint` on `NEED_GENERATION` snapshots; and
...
- the batch;
- exact source kind `NEED_GENERATION` through typed composite batch ownership;
...
WHOLESALE approval snapshots retain null RMVP-07 validation/fingerprint fields. A WHOLESALE batch in `RELEASED_FOR_PURCHASE_HANDOFF`, version `1`, requires neither new pointer nor release row; existing PA-05D behavior remains valid.
...
The server determines action eligibility and returns the exact first matching machine-readable disabled code plus its safe Vietnamese message. The browser cannot infer eligibility from status alone. Exact fields, registries and precedence are frozen by the architecture contract.
