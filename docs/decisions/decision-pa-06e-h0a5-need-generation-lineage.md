...
| Previously `REMOVED` line                                | Terminal; it is not silently treated as an active contribution and need not be repeated in later runs.  |
...
| **H0A5 theoretical-line/source/predecessor/release integrity** | Atomic uniqueness; direct typed source completeness; immutable lines/use relations; exact quantity/unit/disposition; theoretical predecessor matching/nonforking; new/ingredient/quantity/removal outcomes; split/merge/silent-omission rejection; one immutable release header; every-and-only release line and issue membership; exact values/source/predecessor/summary; release immutability and typed downstream FK boundary.     |
