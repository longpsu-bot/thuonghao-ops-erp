...
select is((select row(count(*)::integer, pg_get_functiondef('atlas_core.planning_contract_01_materialize_confirmed_needs(jsonb)'::regprocedure) like '%PANTRY_DIRECT%')::text from atlas_planning.confirmed_need_line_revision_contributions c join atlas_planning.confirmed_need_line_revisions r using(confirmed_need_line_revision_id) where r.is_current and c.need_generation_release_snapshot_id='cb200000-0000-0000-0000-000000000290'),'(6,t)','current membership exactly partitions the release and the shared CMD-15 algorithm owns the closed Pantry compatibility branch');
