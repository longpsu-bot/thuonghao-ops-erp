...
-- 09
select is((select count(*)::integer from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'atlas_core' and p.proname like 'rmvp_03b_%' and pg_get_userbyid(p.proowner) = 'atlas_owner' and not p.prosecdef and p.proconfig = array['search_path=""']::text[]), 20, 'R3B-09 exactly twenty private helpers are atlas_owner invokers');
-- 10
