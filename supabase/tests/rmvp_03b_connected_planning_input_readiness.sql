...
-- 11
select is((select jsonb_build_object('tables', count(*) filter (where c.relkind='r'), 'views', count(*) filter (where c.relkind in ('v','m')), 'rmvp_06_relations', array_agg(c.relname order by c.relname) filter (where c.relkind='r' and n.nspname='atlas_planning' and c.relname like 'confirmed_need_validation_%'), 'rmvp_07_relations', array_agg(c.relname order by c.relname) filter (where c.relkind='r' and n.nspname='atlas_planning' and c.relname='confirmed_need_releases')) from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname like 'atlas\_%' escape '\'), jsonb_build_object('tables',113,'views',2,'rmvp_06_relations',array['confirmed_need_validation_attempts','confirmed_need_validation_issues','confirmed_need_validation_lines']::text[],'rmvp_07_relations',array['confirmed_need_releases']::text[]), 'R3B-11 RMVP-03B itself owns no new relation or view; the current Atlas catalog is exactly 113 tables and 2 views while the bounded RMVP-06/07 relations and private OPS-v1 adoption evidence remain unchanged');
-- 12
