...
-- 13
select is((select jsonb_build_object('capabilities',count(*),'rmvp_06_capabilities',array_agg(capability_code order by capability_code) filter (where capability_code like 'confirmed_need_validation.%'),'rmvp_07_capabilities',array_agg(capability_code order by capability_code) filter (where capability_code in ('confirmed_need_approval.approve','confirmed_need_release.release'))) from atlas_core.capabilities), jsonb_build_object('capabilities',27,'rmvp_06_capabilities',array['confirmed_need_validation.validate']::text[],'rmvp_07_capabilities',array['confirmed_need_approval.approve','confirmed_need_release.release']::text[]), 'R3B-13 current catalogue includes exactly the RMVP-06 validation and two RMVP-07 lifecycle capabilities');
-- 14
select is((select jsonb_build_object('apis',count(*),'rmvp_06_apis',array_agg(p.proname order by p.proname) filter (where p.proname='validate_confirmed_needs'),'rmvp_07_apis',array_agg(p.proname order by p.proname) filter (where p.proname in ('approve_confirmed_needs','release_confirmed_needs_for_purchase_handoff'))) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='atlas_api'), jsonb_build_object('apis',84,'rmvp_06_apis',array['validate_confirmed_needs']::text[],'rmvp_07_apis',array['approve_confirmed_needs','release_confirmed_needs_for_purchase_handoff']::text[]), 'R3B-14 current catalogue includes the exact additive 84-function API total and exact RMVP-06/07 APIs while the four RMVP-03B identities remain asserted separately');
-- 15
