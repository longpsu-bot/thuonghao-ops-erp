...
select is((
  ...
  where need_generation_run_id=(select (response->'affected_aggregate_ids'->>'need_generation_run_id')::uuid from pct01_responses where response_name='execute-initial')
),2::bigint,'PCT01-32 immutable release membership is every generated line');
select is((
...
select is((
  ...
  where confirmed_need_batch_id=(select (response->'affected_aggregate_ids'->>'confirmed_need_batch_id')::uuid from pct01_responses where response_name='execute-initial')
),2::bigint,'PCT01-34 H0C grouping preserves distinct delivery locations');
select is((
  select count(*) from atlas_planning.confirmed_need_line_revision_contributions
  where confirmed_need_batch_id=(select (response->'affected_aggregate_ids'->>'confirmed_need_batch_id')::uuid from pct01_responses where response_name='execute-initial')
),2::bigint,'PCT01-35 H0C membership retains both theoretical contributions');
...
select is((select (response->'new_versions'->>'confirmed_need_batch_version')::bigint from pct01_responses where response_name='execute-correction'),2::bigint,'PCT01-51 Confirmed Need correction advances one version');
