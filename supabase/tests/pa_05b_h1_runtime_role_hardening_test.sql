...
select ok(
  not exists (
    select 1 from unnest(array['atlas_command_runtime', 'atlas_evidence_command_runtime', 'atlas_dispatch_command_runtime', 'atlas_read_runtime']) r(role_name)
...
select is(
  ...
  9,
  'authenticated can execute exactly the nine reviewed PA-05B and PA-05C API functions'
);
...
select is(
  ...
  9,
  'no Atlas API functions beyond the reviewed PA-05B and PA-05C surface exist'
);
