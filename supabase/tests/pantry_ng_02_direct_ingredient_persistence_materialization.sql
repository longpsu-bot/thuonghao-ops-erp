...
-- 40
select is((select jsonb_build_object('tables',count(*),'rmvp_06_relations',array_agg(n.nspname||'.'||c.relname order by n.nspname,c.relname) filter (where n.nspname='atlas_planning' and c.relname like 'confirmed_need_validation_%'),'rmvp_07_relations',array_agg(n.nspname||'.'||c.relname order by n.nspname,c.relname) filter (where n.nspname='atlas_planning' and c.relname='confirmed_need_releases')) from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname like 'atlas\_%' escape '\' and c.relkind='r'),jsonb_build_object('tables',113,'rmvp_06_relations',array['atlas_planning.confirmed_need_validation_attempts','atlas_planning.confirmed_need_validation_issues','atlas_planning.confirmed_need_validation_lines']::text[],'rmvp_07_relations',array['atlas_planning.confirmed_need_releases']::text[]),'PNG02-040 current ordinary-table catalog includes the exact RMVP-06/07 evidence relations, 02B continuity evidence, Direct Need mode facts, School PXK evidence, and private OPS-v1 adoption evidence');
-- 41
...
-- 42
select is((select jsonb_build_object('rmvp_06_apis',array_agg(n.nspname||'.'||p.proname order by n.nspname,p.proname) filter (where p.proname='validate_confirmed_needs'),'rmvp_07_apis',array_agg(n.nspname||'.'||p.proname order by n.nspname,p.proname) filter (where p.proname in ('approve_confirmed_needs','release_confirmed_needs_for_purchase_handoff'))) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='atlas_api'),jsonb_build_object('rmvp_06_apis',array['atlas_api.validate_confirmed_needs']::text[],'rmvp_07_apis',array['atlas_api.approve_confirmed_needs','atlas_api.release_confirmed_needs_for_purchase_handoff']::text[]),'PNG02-042 current physical API catalog includes the exact RMVP-06/07 API identities');
-- 43
select is((select jsonb_build_object('rmvp_06_private_functions',array_agg(n.nspname||'.'||p.proname order by n.nspname,p.proname) filter (where p.proname like 'rmvp_06_%'),'rmvp_07_private_functions',array_agg(n.nspname||'.'||p.proname order by n.nspname,p.proname) filter (where p.proname like 'rmvp_07_%')) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname like 'atlas\_%' escape '\' and n.nspname<>'atlas_api'),jsonb_build_object('rmvp_06_private_functions',array['atlas_core.rmvp_06_canonical_evaluation','atlas_core.rmvp_06_error','atlas_core.rmvp_06_extend_workbench','atlas_core.rmvp_06_issue','atlas_core.rmvp_06_record_change','atlas_core.rmvp_06_validate_command','atlas_planning.rmvp_06_immutable_validation_evidence','atlas_planning.rmvp_06_validation_integrity']::text[],'rmvp_07_private_functions',array['atlas_core.rmvp_07_authorize','atlas_core.rmvp_07_canonical_decimal','atlas_core.rmvp_07_error','atlas_core.rmvp_07_extend_workbench','atlas_core.rmvp_07_record_change','atlas_core.rmvp_07_snapshot_current_complete','atlas_core.rmvp_07_validate_command','atlas_core.rmvp_07_validated_facts_fingerprint','atlas_core.rmvp_07_validated_facts_projection','atlas_core.rmvp_07_validation_evidence_complete','atlas_planning.rmvp_07_approval_release_integrity','atlas_planning.rmvp_07_immutable_approval_release_evidence']::text[]),'PNG02-043 current private-function catalog includes the exact RMVP-06/07 helpers and excludes their public APIs');
-- 44
select is((select jsonb_build_object('capabilities',count(*),'rmvp_06_capabilities',array_agg(capability_code order by capability_code) filter (where capability_code like 'confirmed_need_validation.%'),'rmvp_07_capabilities',array_agg(capability_code order by capability_code) filter (where capability_code in ('confirmed_need_approval.approve','confirmed_need_release.release'))) from atlas_core.capabilities),jsonb_build_object('capabilities',31,'rmvp_06_capabilities',array['confirmed_need_validation.validate']::text[],'rmvp_07_capabilities',array['confirmed_need_approval.approve','confirmed_need_release.release']::text[]),'PNG02-044 current capability catalog includes the exact RMVP-06/07 and School PXK capabilities');
-- 45
...
-- 47
select is((select jsonb_build_object('policies',count(*),'d_042_policies',array_agg(n.nspname||'.'||c.relname||'|'||p.polname order by c.relname,p.polname) filter (where p.polname='issue_222_planning_commitment_select'),'rmvp_06_policies',array_agg(n.nspname||'.'||c.relname||'|'||p.polname order by c.relname,p.polname) filter (where p.polname like 'rmvp_06_%'),'rmvp_07_policies',array_agg(n.nspname||'.'||c.relname||'|'||p.polname order by c.relname,p.polname) filter (where p.polname like 'rmvp_07_%')) from pg_policy p join pg_class c on c.oid=p.polrelid join pg_namespace n on n.oid=c.relnamespace),jsonb_build_object('policies',652,'d_042_policies',array['atlas_procurement.fulfilment_allocations|issue_222_planning_commitment_select']::text[],'rmvp_06_policies',array['atlas_planning.confirmed_need_validation_attempts|rmvp_06_validation_attempt_insert','atlas_planning.confirmed_need_validation_attempts|rmvp_06_validation_attempt_select','atlas_planning.confirmed_need_validation_issues|rmvp_06_validation_issue_insert','atlas_planning.confirmed_need_validation_issues|rmvp_06_validation_issue_select','atlas_planning.confirmed_need_validation_lines|rmvp_06_validation_line_insert','atlas_planning.confirmed_need_validation_lines|rmvp_06_validation_line_select','atlas_planning.need_generation_issues|rmvp_06_need_generation_issue_select','atlas_planning.need_generation_release_snapshot_issues|rmvp_06_need_generation_release_issue_select','atlas_planning.need_generation_release_snapshot_lines|rmvp_06_need_generation_release_line_lock','atlas_planning.need_generation_release_snapshots|rmvp_06_need_generation_release_lock','atlas_planning.need_generation_runs|rmvp_06_need_generation_run_lock','atlas_planning.planning_quantity_policy_revisions|rmvp_06_policy_revision_lock']::text[],'rmvp_07_policies',array['atlas_planning.confirmed_need_approval_snapshots|rmvp_07_approval_snapshot_insert','atlas_planning.confirmed_need_approval_snapshots|rmvp_07_approval_snapshot_lock','atlas_planning.confirmed_need_approval_snapshots|rmvp_07_approval_snapshot_select','atlas_planning.confirmed_need_line_decisions|rmvp_07_decision_lock','atlas_planning.confirmed_need_line_revision_contributions|rmvp_07_contribution_lock','atlas_planning.confirmed_need_releases|rmvp_07_release_insert','atlas_planning.confirmed_need_releases|rmvp_07_release_lock','atlas_planning.confirmed_need_releases|rmvp_07_release_select','atlas_planning.confirmed_need_snapshot_lines|rmvp_07_snapshot_line_insert','atlas_planning.confirmed_need_snapshot_lines|rmvp_07_snapshot_line_lock','atlas_planning.confirmed_need_snapshot_lines|rmvp_07_snapshot_line_select','atlas_planning.confirmed_need_validation_attempts|rmvp_07_validation_attempt_lock','atlas_planning.confirmed_need_validation_issues|rmvp_07_validation_issue_lock','atlas_planning.confirmed_need_validation_lines|rmvp_07_validation_line_lock','atlas_planning.purchase_handoff_batches|rmvp_07_purchase_handoff_select']::text[]),'PNG02-047 current policy catalog includes the exact D-042, relation-owned RMVP-06/07, accepted 02B, Direct Need mode, School PXK, and OPS-v1 adoption policies');
-- 48
...
-- 49
select is((select jsonb_build_object('ordinary_triggers',count(*),'purchase_review_ordinary_triggers',array_agg(n.nspname||'.'||c.relname||'|'||t.tgname order by c.relname,t.tgname) filter (where t.tgname like 'purchase_review_%' and t.tgenabled='O'),'rmvp_06_ordinary_triggers',array_agg(n.nspname||'.'||c.relname||'|'||t.tgname order by c.relname,t.tgname) filter (where t.tgname in ('confirmed_need_validation_attempts_immutable','confirmed_need_validation_issues_immutable','confirmed_need_validation_lines_immutable')),'rmvp_07_ordinary_triggers',array_agg(n.nspname||'.'||c.relname||'|'||t.tgname order by c.relname,t.tgname) filter (where t.tgname like '%rmvp07_immutable')) from pg_trigger t join pg_class c on c.oid=t.tgrelid join pg_namespace n on n.oid=c.relnamespace where not t.tgisinternal and t.tgconstraint=0 and n.nspname like 'atlas\_%' escape '\'),jsonb_build_object('ordinary_triggers',63,'purchase_review_ordinary_triggers',array['atlas_planning.confirmed_need_batches|purchase_review_confirmed_need_release_readiness']::text[],'rmvp_06_ordinary_triggers',array['atlas_planning.confirmed_need_validation_attempts|confirmed_need_validation_attempts_immutable','atlas_planning.confirmed_need_validation_issues|confirmed_need_validation_issues_immutable','atlas_planning.confirmed_need_validation_lines|confirmed_need_validation_lines_immutable']::text[],'rmvp_07_ordinary_triggers',array['atlas_planning.confirmed_need_approval_snapshots|confirmed_need_approval_snapshots_rmvp07_immutable','atlas_planning.confirmed_need_releases|confirmed_need_releases_rmvp07_immutable','atlas_planning.confirmed_need_snapshot_lines|confirmed_need_snapshot_lines_rmvp07_immutable']::text[]),'PNG02-049 current ordinary-trigger catalog includes the exact RMVP-06/07, 02B, Direct Need mode, School PO/PXK, OPS-v1 evidence immutability, and enabled purchase-review release guards');
-- 50
select is((select jsonb_build_object('constraint_triggers',count(*),'purchase_review_deferred_constraint_triggers',array_agg(n.nspname||'.'||c.relname||'|'||t.tgname order by c.relname,t.tgname) filter (where t.tgname like 'purchase_review_%' and t.tgenabled='O' and con.condeferrable and con.condeferred),'rmvp_06_deferred_constraint_triggers',array_agg(n.nspname||'.'||c.relname||'|'||t.tgname order by c.relname,t.tgname) filter (where t.tgname in ('confirmed_need_batches_validation_integrity','confirmed_need_validation_attempts_integrity','confirmed_need_validation_issues_integrity','confirmed_need_validation_lines_integrity') and con.condeferrable and con.condeferred),'rmvp_07_deferred_constraint_triggers',array_agg(n.nspname||'.'||c.relname||'|'||t.tgname order by c.relname,t.tgname) filter (where t.tgname like '%rmvp07_integrity' and con.condeferrable and con.condeferred)) from pg_trigger t join pg_class c on c.oid=t.tgrelid join pg_namespace n on n.oid=c.relnamespace join pg_constraint con on con.oid=t.tgconstraint where not t.tgisinternal and t.tgconstraint<>0 and n.nspname like 'atlas\_%' escape '\'),jsonb_build_object('constraint_triggers',49,'purchase_review_deferred_constraint_triggers',array['atlas_procurement.school_catering_allocation_family_contributions|purchase_review_contribution_integrity','atlas_procurement.school_catering_allocation_family_revisions|purchase_review_revision_integrity','atlas_procurement.school_catering_allocation_supplier_splits|purchase_review_split_integrity']::text[],'rmvp_06_deferred_constraint_triggers',array['atlas_planning.confirmed_need_batches|confirmed_need_batches_validation_integrity','atlas_planning.confirmed_need_validation_attempts|confirmed_need_validation_attempts_integrity','atlas_planning.confirmed_need_validation_issues|confirmed_need_validation_issues_integrity','atlas_planning.confirmed_need_validation_lines|confirmed_need_validation_lines_integrity']::text[],'rmvp_07_deferred_constraint_triggers',array['atlas_planning.confirmed_need_approval_snapshots|confirmed_need_approval_snapshots_rmvp07_integrity','atlas_planning.confirmed_need_batches|confirmed_need_batches_rmvp07_integrity','atlas_planning.confirmed_need_releases|confirmed_need_releases_rmvp07_integrity','atlas_planning.confirmed_need_snapshot_lines|confirmed_need_snapshot_lines_rmvp07_integrity']::text[]),'PNG02-050 current constraint-trigger catalog includes the exact RMVP-06/07, 02B and enabled purchase-review deferrable initially-deferred integrity triggers');
-- 51
...
-- 67
select ok((select pg_get_functiondef('atlas_core.direct_need_evaluation_ready(uuid,date,date)'::regprocedure) like all(array['%pantry.pantry_need_batch_status=''APPROVED''%','%pantry.version=e.pantry_need_batch_version%','%pantry.latest_approval_snapshot_id=e.pantry_need_approval_snapshot_id%'])),'PNG02-067 shared readiness predicate keeps the Pantry header current and approved');
-- 68
select ok((select pg_get_functiondef('atlas_core.direct_need_evaluation_ready(uuid,date,date)'::regprocedure) like all(array['%pantry.week_start<=target_period_start%','%pantry.week_end>=target_period_end%','%pantry_snapshot.line_count%'])),'PNG02-068 shared readiness predicate guards ownership, header count, and exact-period containment');

-- Exact-period inclusion and exclusion (69-77).
-- 69
select ok((select pg_get_functiondef('atlas_core.direct_need_evaluation_ready(uuid,date,date)'::regprocedure) like '%pantry_snapshot.line_count=(select count(*)%'),'PNG02-069 shared readiness predicate retains and counts an explicit zero-line Pantry header');
-- 70
