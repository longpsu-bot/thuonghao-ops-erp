...
select pg_temp.proposal_seed_run(
  ...
  jsonb_build_array(
    jsonb_build_object('theoretical_id','d4600000-0000-0000-0000-000000001101','service_date','2026-09-17','ingredient_id','d4600000-0000-0000-0000-000000000101','unit_id','d4600000-0000-0000-0000-000000000014','quantity','0.004'),
    jsonb_build_object('theoretical_id','d4600000-0000-0000-0000-000000001102','service_date','2026-09-17','ingredient_id','d4600000-0000-0000-0000-000000000101','unit_id','d4600000-0000-0000-0000-000000000014','quantity','0.004'),
    ...
    jsonb_build_object('theoretical_id','d4600000-0000-0000-0000-000000001108','service_date','2026-09-17','ingredient_id','d4600000-0000-0000-0000-000000000107','unit_id','d4600000-0000-0000-0000-000000000015','quantity','204.75')
  )
);
...
select is((select response#>>'{result_counts,created_confirmed_need_line_count}' from proposal_results where result_name='examples'), '7', 'POP-02 eight contributions form seven operational identities');
select is((select row(theoretical_quantity, confirmed_quantity)::text from atlas_planning.confirmed_need_line_revisions where ingredient_id='d4600000-0000-0000-0000-000000000101'), '(0.008000,0.010000)', 'POP-03 aggregate 0.004 + 0.004 is quantized once to 0.01');
select is((select row(theoretical_quantity, confirmed_quantity)::text from atlas_planning.confirmed_need_line_revisions where ingredient_id='d4600000-0000-0000-0000-000000000102'), '(0.025500,0.030000)', 'POP-04 raw 0.0255 kg proposes 0.03');
select is((select row(theoretical_quantity, confirmed_quantity)::text from atlas_planning.confirmed_need_line_revisions where ingredient_id='d4600000-0000-0000-0000-000000000103'), '(0.105000,0.110000)', 'POP-05 raw 0.105 kg proposes 0.11');
select is((select row(theoretical_quantity, confirmed_quantity)::text from atlas_planning.confirmed_need_line_revisions where ingredient_id='d4600000-0000-0000-0000-000000000104'), '(1.225000,1.230000)', 'POP-06 raw 1.225 kg proposes 1.23');
...
select is((select row(theoretical_quantity, confirmed_quantity)::text from atlas_planning.confirmed_need_line_revisions where ingredient_id='d4600000-0000-0000-0000-000000000107'), '(204.750000,205.000000)', 'POP-09 raw count 204.75 proposes 205');
select is((select count(*) from atlas_planning.confirmed_need_line_revisions r where r.theoretical_quantity=(select sum(c.controlled_contribution_quantity) from atlas_planning.confirmed_need_line_revision_contributions c where c.confirmed_need_line_revision_id=r.confirmed_need_line_revision_id)), 7::bigint, 'POP-10 every raw revision total remains its exact membership sum');
select is((select jsonb_agg(theoretical_quantity order by theoretical_need_line_id) from atlas_planning.theoretical_need_lines where need_generation_run_id='d4600000-0000-0000-0000-000000001000'), '[0.004000,0.004000,0.025500,0.105000,1.225000,228.000000,3.800000,204.750000]'::jsonb, 'POP-11 materialization does not rewrite theoretical source quantities');
select is((select count(*) from atlas_planning.theoretical_need_lines where need_generation_run_id='d4600000-0000-0000-0000-000000001000'), 8::bigint, 'POP-12 materialization neither adds nor removes theoretical lines');
  ...
  select jsonb_agg(jsonb_build_object(
    ...
    'proposed_confirmed_quantity', case
      when p_mode in ('VALID', 'INVALID_KG')
        and line.ingredient_id = 'd4600000-0000-0000-0000-000000000102'
        then case when p_mode = 'VALID' then '0.04' else '0.025' end
      when p_mode = 'INVALID_COUNT'
        and line.ingredient_id = 'd4600000-0000-0000-0000-000000000106'
        then '3.8'
      when p_mode = 'VALID'
        and line.ingredient_id = 'd4600000-0000-0000-0000-000000000104'
        then '1.230000'
    ...
    'reason_note', case
      ...
      when p_mode = 'INVALID_COUNT'
        and line.ingredient_id = 'd4600000-0000-0000-0000-000000000106'
        then 'Invalid count adjustment fixture'
      else null
    end
...
select ok((select response->>'success'='true' and response#>>'{workbench,line_counts,total}'='7' from proposal_results where result_name='review'), 'POPD-01 readback exposes all seven fresh proposal lines');
...
select is((select jsonb_object_agg(decision_kind, line_count) from (select decision_kind, count(*)::integer line_count from atlas_planning.confirmed_need_line_decisions where confirmed_need_batch_id=(select (response#>>'{affected_aggregate_ids,confirmed_need_batch_id}')::uuid from proposal_results where result_name='examples') group by decision_kind) counts), jsonb_build_object('ADJUSTED_QUANTITY_CONFIRMED',1,'UNCHANGED_PROPOSAL_ACCEPTED',6), 'POPD-09 first Save creates six accepted proposals and one explicit adjustment');
select is((select jsonb_object_agg(reason_code, line_count) from (select reason_code, count(*)::integer line_count from atlas_planning.confirmed_need_line_decisions where confirmed_need_batch_id=(select (response#>>'{affected_aggregate_ids,confirmed_need_batch_id}')::uuid from proposal_results where result_name='examples') group by reason_code) counts), jsonb_build_object('OPERATIONAL_QUANTITY_ADJUSTMENT',1,'PROPOSAL_ACCEPTED',6), 'POPD-10 system quantization never creates a human Planning-step adjustment');
select is((select count(*) from atlas_planning.confirmed_need_line_decisions where confirmed_need_batch_id=(select (response#>>'{affected_aggregate_ids,confirmed_need_batch_id}')::uuid from proposal_results where result_name='examples') and decision_number=1 and predecessor_decision_id is null), 7::bigint, 'POPD-11 every fresh line receives exactly one first decision');
select is((select row(theoretical_quantity,confirmed_quantity)::text from atlas_planning.confirmed_need_line_revisions where ingredient_id='d4600000-0000-0000-0000-000000000102' and revision_number=1), '(0.025500,0.030000)', 'POPD-12 Save preserves the raw quantity and generated system proposal revision');
...
select is((select confirmed_quantity from atlas_planning.confirmed_need_line_revisions where need_generation_run_id='d4600000-0000-0000-0000-000000002200' and is_current), 0.110000::numeric, 'POP-19 correction derives the new proposal from current policy');
