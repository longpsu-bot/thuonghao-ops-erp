...
select is((select count(*)::integer from information_schema.role_routine_grants where grantee='atlas_planning_materialization_runtime' and privilege_type='EXECUTE'), 20, 'runtime has exactly the twenty practical function executes after the bounded adoption repair');
...
select is((select count(*)::integer from information_schema.role_table_grants where grantee='atlas_planning_materialization_runtime' and privilege_type='SELECT'), 51, 'runtime selects exactly the approved fifty-one relations');
...
select is((select count(*)::integer from pg_policy p join pg_roles r on r.oid=any(p.polroles) where r.rolname='atlas_planning_materialization_runtime'), 65, 'runtime has exactly sixty-five verb-specific policies');
select is((select count(*)::integer from pg_policy p join pg_roles r on r.oid=any(p.polroles) where r.rolname='atlas_planning_materialization_runtime' and p.polcmd='r'), 53, 'runtime has fifty-three SELECT policies');
...
select is(
  ...
  jsonb_build_object(
    'historical_functions', array[
      ...
      'atlas_core.direct_need_prepare_evaluation(request jsonb, actor_id uuid, target_start date, target_end date, preflight jsonb)',
      'atlas_core.direct_need_replace_modes(target_week_start date, canonical_rows jsonb, supplied_modes jsonb, actor_id uuid)',
  ...
  'planning command runtime retains the exact historical, D-042, and D-044 helper function set, the three accepted source-completion APIs, and exactly three RMVP-03B command functions'
);
select is(
  jsonb_build_object(
