...
with released_mismatches as materialized (
  select version.recipe_version_id,line.recipe_line_revision_id,line.recipe_line_id,
...
export function planningAdoptionPostDeployQuerySql() {
  ...
  select distinct target_recipe_version_id recipe_version_id from evidence
...
select jsonb_build_object(
  ...
  'excluded_native_source_kind','UIQ03A_SAVE',
  'incomplete_candidate_count',(select count(*)::integer from mapped where mapped_revision_id is null or legacy_ingredient_id is null or complete_batch_id is null),
  ...
  'remapped_source_count',(select count(*)::integer from mapped where mapped_batch_id<>import_batch_id or mapped_fingerprint<>source_fingerprint),
  'correction_evidence_count',(select count(*)::integer from evidence),
  'direct_successor_version_count',(select count(*)::integer from atlas_admin.recipe_versions target join successors on successors.recipe_version_id=target.recipe_version_id join atlas_admin.recipe_versions predecessor on predecessor.recipe_version_id=target.predecessor_recipe_version_id),
  'successor_present_count',(select count(*)::integer from present),
  'exact_sibling_copy_count',(select count(*)::integer from present p where not exists(select 1 from corrected c where c.recipe_line_revision_id=p.recipe_line_revision_id)),
  'locked_predecessor_version_count',(select count(distinct predecessor.recipe_version_id)::integer from atlas_admin.recipe_versions target join successors on successors.recipe_version_id=target.recipe_version_id join atlas_admin.recipe_versions predecessor on predecessor.recipe_version_id=target.predecessor_recipe_version_id where predecessor.recipe_version_status='LOCKED')
)`;
}
