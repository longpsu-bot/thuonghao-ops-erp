...
describe("Atlas vNext presentation boundary", () => {
  it("permits only reviewed School master-data modules through its bridge", () => {
    for (const module of ["masterDataApi", "masterDataModel"]) {
      ...
      expect(
        checkSources({ "src/vnext/atlas/bridges/schoolMasterData.ts": source }),
      ).toEqual([]);
      expect(
        checkSources({ "src/vnext/atlas/schools/View.tsx": source }),
      ).toHaveLength(1);
    }
    for (const module of [
      ...
      expect(
        checkSources({
          "src/vnext/atlas/bridges/schoolMasterData.ts": `export * from "${module}";`,
        }),
      ).toHaveLength(1);
    }
  });
  it("permits reconciliation business modules only through a bridge and rejects presentation", () => {
    for (const module of [
      ...
      expect(
        checkSources({ "src/vnext/atlas/bridges/schoolFulfilment.ts": source }),
      ).toEqual([]);
      expect(
        checkSources({ "src/vnext/atlas/reconciliation/View.tsx": source }),
      ).toHaveLength(1);
    }
    for (const module of [
      "SchoolFulfilmentReconciliationWorkbench",
      "reviewSchoolFulfilmentReconciliationApi",
    ]) {
      expect(
        checkSources({
          "src/vnext/atlas/bridges/schoolFulfilment.ts": `export * from "@/modules/atlas/dispatch/${module}";`,
        }),
      ).toHaveLength(1);
    }
  });
