...
export function historicalPantryMessage(item: ReadinessHistoryItem) {
  return item.evaluation?.historical_pantry_state === "PRE_PANTRY_NULL_BINDING"
    ? "Trạng thái lịch sử được ghi nhận trước khi có Nhu cầu bổ sung; không thể dùng để tạo nhu cầu."
    : null;
}
...
export function readinessResultMessage(result: AtlasRpcResult) {
  ...
  if (result.kind === "transport_error")
    return "Chưa xác định kết quả thao tác. Atlas không tự động gửi lại; hãy tải lại trạng thái mới nhất.";
  if (result.kind === "client_error")
    return "Ứng dụng không thể kiểm tra trạng thái dữ liệu đầu vào.";
  const messages: Record<string, string> = {
    ...
    STALE_ROOT_STATE:
      "Trạng thái dữ liệu đã thay đổi. Hãy tải lại trước khi tiếp tục.",
  ...
  return (
    ...
    "Không thể cập nhật trạng thái dữ liệu đầu vào. Hãy tải lại trước khi tiếp tục."
  );
}
