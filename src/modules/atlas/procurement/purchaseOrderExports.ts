...
export function buildPurchaseOrderPdfDefinition(
  ...
  return {
    info: { title: `Đơn mua ${data.documentNumber}` },
    content: [
      { text: "THƯỢNG HẢO · ĐƠN MUA", style: "heading" },
...
export async function createPurchaseOrderXlsx(
  ...
  const summary = workbook.addWorksheet("Đơn mua");
  summary.addRow(["THƯỢNG HẢO · ĐƠN MUA"]);
  ...
  summary.addRow(["Phiên bản phát hành", String(data.releasedRevision)]);
  summary.addRow([]);
  ...
  data.summaryLines.forEach((line) =>
    summary.addRow([line.ingredientName, line.orderedQuantity, line.unitCode]),
  );
  ...
  const schoolHeader = bySchool.addRow([
    "Trường / điểm giao",
    "Nguyên liệu",
    "Số lượng đặt",
    "Đơn vị",
  ]);
  styleWorksheetHeader(schoolHeader);
  data.schoolLines.forEach((line) =>
    bySchool.addRow([
      line.locationName,
      line.ingredientName,
      line.orderedQuantity,
  ...
  bySchool.getColumn(1).width = 34;
  bySchool.getColumn(2).width = 30;
  bySchool.getColumn(3).width = 18;
  bySchool.getColumn(4).width = 12;
...
export async function downloadPurchaseOrderPdf(
  ...
  downloadBytes(bytes, "application/pdf", `${order.document_number}.pdf`);
}
