...
PA-05D is the next proposed backend command family. It is documentation-only until approved and implements the Planning-owned start of the supplier-direct wholesale path: record wholesale source, release wholesale order with pass-through Confirmed Need, release Purchase Handoff, and release Dispatch Requirement. It deliberately excludes Procurement allocation/PO, Dispatch plan/trip setup, UI, deployment, seed data, and OPS v1 mutation. After PA-05D, the planned backend sequence is PA-05E Procurement commands, PA-05F Dispatch setup commands, and PA-05G backend end-to-end acceptance before PA-06 React connection.
