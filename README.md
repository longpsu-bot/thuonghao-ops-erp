...
PA-05D and PA-05E are complete on `main`. Together they add the authoritative Planning release chain, exact supplier-direct fulfilment allocation, and released supplier purchase orders while preserving the reviewed 15-function API boundary. Capability review then identified one remaining prerequisite before Dispatch setup: the existing PA-05B load and successful-delivery commands were validated around one load line per stop, while PA-05D and PA-05E now create normal multi-line requirements and allocations. PA-05B-H2 therefore corrects the existing Dispatch load, departure, and successful-delivery commands for atomic multi-line execution without adding a public function or table. The backend sequence is PA-05B-H2, PA-05F Dispatch setup, PA-05G end-to-end backend acceptance, then PA-06 React connection.
