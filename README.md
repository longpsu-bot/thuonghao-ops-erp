...
PA-05D, PA-05E, and PA-05B-H2 are complete on `main`. Together they add the authoritative Planning release chain, exact supplier-direct fulfilment allocation, released supplier purchase orders, atomic multi-line loading, trip-wide departure revalidation, and multi-line successful delivery while preserving the reviewed 15-function API boundary. PA-05F is the current bounded contract and implementation gate under Issue #95: it adds command-authored Dispatch Plan, Plan Requirement, assigned Trip, and exact Stop setup without routing optimization or another runtime role. The approved PA-05A catalog also retains a separate successful-trip closure command, tracked by Issue #93, so PA-05G remains acceptance-only rather than hiding a missing write command. The backend sequence is PA-05F Dispatch setup, PA-05B-H3 successful trip closure, PA-05G end-to-end backend acceptance, then PA-06 React connection.
...
No Supabase credential or production-data access is required. The merged PA-04 through PA-05B-H2 migrations can be verified against a disposable local PostgreSQL/Supabase-compatible database without linking a hosted project.
...
17. [`PA-05F — Bounded Dispatch setup command family`](docs/architecture/pa-05f-dispatch-setup-command-family-contract.md) — exact Dispatch Plan, Plan Requirement, assigned Trip, and derived Stop setup
